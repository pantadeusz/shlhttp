...
public:
  ...
  request_t &set_body(const std::string &b) {
    body = std::vector<char>(b.begin(), b.end());
    return *this;
  }
};
...
auto response_of = [](int status_code,std::string status_message, std::string body )->response_t{
    response_t response;
...
auto listen_socket = [](std::string host = "localhost",
                        std::string port = "8080") -> int {
  int yes = 1;
...
auto accept_on_socket =
    [](int sfd) -> std::tuple<int, std::string, std::string> {
...
auto receive_request = [](int sockfd) {
  ...
  if (request.method == "GET")
    return request;
  if (request.headers["content-length"].size() > 0) {
    int content_length = std::stoi(request.headers["content-length"]);
    for (int i = 0; (i < content_length) && (recv(sockfd, buf, 1, 0) > 0);
         i++) {
      input_buffer.push_back(buf[0]);
    }
    std::stringstream ss(input_buffer);
